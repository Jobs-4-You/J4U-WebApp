...
function JobResultList({ recomContainer, rank, avam, classes }) {
  ...
  try {
    ...
    const Center = {
      textAlign: "center",
      display:
        recomContainer.state.totalCounts[jobIndex] &&
        recomContainer.state.openPositions[jobIndex]
          ? "block"
          : "none"
    };
    ...
    if (recomContainer.state.openPositions) {
      return (
        <List component="nav" className={classes.root}>
          ...
          <div style={noResults}>
            <Typography>
              Nous n’avons actuellement aucune offre correspondant à ce poste
              spécifique. Retentez plus tard, une offre pourrait apparaître.
            </Typography>
          </div>
        </List>
      );
